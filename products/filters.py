...
class ProductFilter(django_filters.FilterSet):
    ...
    def filter_multiple_price_ranges(self, queryset, name, value):
        ...
        if "50_100" in value:
            queries |= Q(sale_price__gte=50, sale_price__lte=100) | Q(
                sale_price__isnull=True, price__gte=50, price__lte=100
            )
        if "over_100" in value:
            queries |= Q(sale_price__gte=100) | Q(
                sale_price__isnull=True, price__gte=100
            )
